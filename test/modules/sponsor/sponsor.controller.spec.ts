...
describe('Sponsor Controller', () => {
  ...
  beforeEach(async () => {
    ...
    sendMailToNewSponsorUseCase = moduleRef.get<SendMailToNewSponsorUseCase>(
      SendMailToNewSponsorUseCase,
    );
    jest.clearAllMocks();
  });
  ...
  it('Should call sendMailToNewSponsorUseCase with correct values', async () => {
    ...
    expect(sendMailToNewSponsorUseCase.send).toHaveBeenCalledWith(
      makeSendMailDto(),
    );
  });
});
